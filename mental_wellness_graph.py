...
def suggest_movie(state: WellnessState) -> dict:
    response = llm.invoke(
        f"You are a movie suggest specialist. "
        f"The user feels: '{state.user_feeling}'. "
        f"Suggest ONE specific movie that would help. "
        f"Include the movie name, about movie (3-4 steps), and how long it takes. "
        f"Keep it under 5 sentences."
    )
    return {
        "movie_suggestion": response.content,
        "messages": [f"[suggest_movie] Done"]
    }
...
def quick_practice(state: WellnessState) -> dict:
    response = llm.invoke(
        ...
        f"Movie:    {state.movie_suggestion}\n"
...
def deep_practice(state: WellnessState) -> dict:
    response = llm.invoke(
        ...
        f"BREATHING: {state.breathing_suggestion}\n"
        f"Movie: {state.movie_suggestion}n"
        f"MINDFULNESS: {state.mindfulness_suggestion}\n"
        f"MOVEMENT: {state.movement_suggestion}\n\n"
        f"Structure it in 4 phases: Settle (breathing), Ground (mindfulness), Release (movement),React (movie). "
...
graph.add_edge("understand_mood", "suggest_movie")
graph.add_edge("suggest_movie", "pick_best_practice")
