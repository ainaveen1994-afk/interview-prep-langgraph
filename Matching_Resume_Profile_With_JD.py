...
def final_decision(state: ResumeMatchState) -> dict:
    ...
    # ✅ Decision logic (based on core skills only)
    if missing.get("core"):
        decision = "REJECTED"
    ...
    # ✅ Format missing skills properly
    missing_text = f"""
Core Skills:
{chr(10).join([f"✘ {m}" for m in missing.get("core", [])])}

AI Skills:
{chr(10).join([f"✘ {m}" for m in missing.get("ai", [])])}

Tools:
{chr(10).join([f"✘ {m}" for m in missing.get("tools", [])])}
"""

    # ✅ Reason (LLM)
    reason = llm.invoke(
        f"""
Explain in 2 lines why candidate is {decision}.

Score: {state.match_score}

Missing Core: {missing.get("core")}
Missing AI: {missing.get("ai")}
Missing Tools: {missing.get("tools")}
"""
    ).content
...
def run_resume_match(jd_path, resume_path, excel_path=None):
    ...
    if os.path.isdir(resume_path):
        ...
        for file in files:
            ...
            # ✅ Save each result into SAME Excel
            save_to_excel(result, file, excel_path)
